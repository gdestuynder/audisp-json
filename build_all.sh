...
docker run --rm -ti -v $(pwd):/build ubuntu /bin/bash -c "apt-get update && apt-get install -y make && cd /build && make deb-deps" && \
  mv audisp-json-${VERSION}-1.x86_64.deb release/audisp-json-${VERSION}-1.x86_64-ubuntu.deb
#sudo make clean || exit
...
hub release edit -m "Releasing ${VERSION}" -a release/audisp-json-${VERSION}-1.x86_64-ubuntu.deb ${VERSION}
